...
| **其他 Mihomo / Clash Meta 客户端** | 🟡 Compatibility entry | [查看兼容说明](docs/kernels/mihomo.md#其他-mihomo--clash-meta-客户端) |
...
Hrules 公共 Mihomo 产品收敛为 **标准版 / 精细版** 两种模式。两者共用同一套 Hrules Core 和规则文件，区别不是安全等级，而是**同一批精细规则最终向用户暴露多少独立出口控制点**。

> **标准版面向公开实用：把复杂 Core 归并成少量高价值场景。精细版面向能力展示与高级控制：保持 AI 等大类不做无意义拆分，重点展示金融业务的细粒度识别，并独立 Apple / iCloud。**

- **标准版 Standard**：按界面顺序提供 `🌐 海外应用 [场景]`、`📺 流媒体 [场景]`、`🤖 AI 服务 [场景]`、`💳 金融服务 [场景]`、`🚀 漏网之鱼 [自选]`。YouTube、Telegram、普通国际应用以及 Apple / iCloud 归入海外应用；Claude / ChatGPT / OpenAI 等归入 AI 服务；银行、券商、支付 / 跨境金融、虚拟货币统一归入金融服务。
- **精细版 Fine-grained**：保留 `🌐 海外应用`、`📺 流媒体`、`🤖 AI 服务`，新增独立 `🍎 Apple / iCloud`，并把金融服务拆分为 `🏦 银行服务`、`📈 证券 / 券商`、`💳 支付 / 跨境金融`、`💰 虚拟货币`。精细版用于展示 Hrules 的业务识别与出口控制精度，也适合确实需要这些独立出口的用户。
- **排序原则**：用户界面从范围最大的普通场景开始，越往后越偏向需要更严格出口控制的业务；`🚀 漏网之鱼` 始终放在最后。规则实际匹配顺序仍遵循“具体规则优先、宽泛规则靠后”，不机械跟随 UI 排序。
- **兼容说明**：精细版暂沿用原 `hrules-strict.js` / `hrules-strict.yaml` 文件名与 Raw URL，避免现有接入链接失效；产品名称与文档统一称“精细版”。
...
两种模式采用同一套 DNS 基础；**标准版** 更偏向保守兼容，**精细版** 保持节点启动解析独立，并使用加密海外 DoH。
...
当前已经完成 **Clash Verge Rev** 多订阅真实客户端验证，以及 **3X-UI Remote Routing → Mihomo** 真实链路验证。其他 Mihomo / Clash Meta 客户端当前只提供兼容入口，完成对应客户端实测后再标记为 Available。**Shadowrocket v0.1 已恢复当前 Core 规则生成与发布链，正在进行最终真实客户端验收，因此暂标记为 Validating。** Shadowrocket 采用单一完整配置，不跟随 Mihomo 的标准版 / 精细版划分。其他客户端会在完成实现与真实验收后逐步开放。
